...
part1 := sum([x |
    x := count(union(answers[_]))
])

part2 := sum([x |
    x := count(intersection(answers[_]))
])

# answers generates an array of sets of sets where each set
# in the array represents the answers for a group and each inner
# set represents the answers for an individual in the group.
answers := [s |
    group := split(raw_groups[_], "\n")
    s := {a |
        g := group[_]
        a := {x | x := split(g, "")[_]}
    }
