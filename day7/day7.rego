...
part1 := count({a |
    contains[a]
    a != "shiny gold"
    nodes := graph.reachable(contains, {a})
    nodes["shiny gold"]
})

part2 := count_upto_8_nested_bags["shiny gold"]

count_upto_8_nested_bags[x] = result {
    some x
    contains_n[x]
    result := sum([s |
        some n1, x1
...
contains_n := {parent: children |
    ...
    children := {[n, s2] |
        ...
        s2 != "no other"
        n := to_number(substring(raw_child, 0, 1))
    }
}
...
input_other := `shiny gold bags contain 2 dark red bags.
...
input_small := `light red bags contain 1 bright white bag, 2 muted yellow bags.
...
input_big := `dim red bags contain 2 bright gold bags, 5 striped fuchsia bags.
