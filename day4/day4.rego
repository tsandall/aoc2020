...
part2 := count([1 |
    ...
    passports[i]
    has_expected_fields[i]
    has_valid_birth_year[i]
    has_valid_issue_year[i]
    has_valid_expiration_year[i]
    has_valid_height[i]
    has_valid_hair_color[i]
    has_valid_eye_color[i]
    has_valid_passport_id[i]
])
...
has_valid_height[i] {
    some i
    p := passports[i]
    [n, unit] := parse_height(p.hgt)
    [min, max] := valid_unit[unit]
    n >= min
    n <= max
}
...
has_valid_passport_id[i] {
    some i
    p := passports[i]
    regex.match("^[0-9]{9}$", p.pid)
}

valid_eye_color := {"amb", "blu", "brn", "gry", "grn", "hzl", "oth"}

parse_height(s) = [n, unit] {
    n := to_number(substring(s, 0, count(s)-2))
    unit := substring(s, count(s)-2, -1)
}
...
required_fields := expected_fields - {"cid"}

passports := [p |
    x := raw_normalized_passports[_]
    p := {k: v |
        [k, v] := split(split(x, " ")[_], ":")
    }
]

raw_normalized_passports := [s | s := replace(raw_passports[_], "\n", " ")]

raw_passports := split(input, "\n\n")

input := input_big

input_small := `ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
input_big := `hcl:5d90f0 cid:270 ecl:#66dc9c hgt:62cm byr:1945 pid:63201172 eyr:2026
